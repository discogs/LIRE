...
public class ImageSearchHitsForEvaluation implements ImageSearchHits {
    ...
    public ImageSearchHitsForEvaluation(Collection<SimpleResultForEvaluation> results, double maxDistance) {
        this.results = new ArrayList<SimpleResultForEvaluation>(results.size());
...
//        }
    ...
    public ImageSearchHitsForEvaluation(Collection<SimpleResultForEvaluation> results, double maxDistance, boolean useSimilarityScore) {
        ...
        // this step normalizes and inverts the distance ...
        // although its now a score or similarity like measure its further called distance
        for (Iterator<SimpleResultForEvaluation> iterator = this.results.iterator(); iterator.hasNext(); ) {
            SimpleResultForEvaluation result = iterator.next();
